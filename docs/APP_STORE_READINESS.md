...
- Live `privacy.html`: returned 404 before this sync because the page had not reached `main /docs`

## Next Ten Steps

1. **Reconcile the branch split.** `origin/main` has release automation fixes through PR #90, while `origin/nightly` has newer v1 product/docs work through PR #87. Decide whether to back-merge main release automation into nightly, promote nightly forward, or stage a controlled release branch before final submission.
